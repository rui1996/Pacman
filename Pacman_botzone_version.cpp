...
namespace Exia
{
    ...
    int Dfs(Pacman::GameField &G, II now, II target, int step) {
        ...
        for (int dir = 3; dir >= 0; dir--) {
            if (G.fieldStatic[now.first][now.second] & (1 << dir))      continue;   //撞墙
            II nxt;
    ...
    int Find_road(Pacman::GameField &G, II now, II target) {
        ...
        for (int i = 3; i >= 0; i--) {
            ...
            if (G.fieldStatic[now.first][now.second] & (1 << i))      continue;   //撞墙
        ...
        for (int i = 3; i >= 0; i--) {
            ...
            for (int k = 0; k < MAX_PLAYER_COUNT; k++) {
                ...
                for (int dir = 3; dir >= 0; dir--) {
                    ...
                    if (G.fieldStatic[enemy_now.first][enemy_now.second] & (1 << dir))      continue;   //撞墙
                    II enemy_nxt;
